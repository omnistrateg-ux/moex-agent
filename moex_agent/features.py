...
def compute_bollinger(close: pd.Series, period: int = 20, std_dev: float = 2.0) -> tuple:
    ...
    # Position within bands (0-1, >1 above, <0 below)
...
def build_feature_frame(candles_1m: pd.DataFrame) -> pd.DataFrame:
    """Return per-timestamp features per secid.

    Input columns: secid, ts, open, high, low, close, value, volume
    """
    df = candles_1m.copy()
    ...
    for secid, g in df.groupby("secid", sort=False):
        ...
        # ATR & normalized distance from rolling VWAP
        g_feat["atr_14"] = compute_atr(g, 14)
        vwap_30 = (g["value"].astype(float).rolling(30).sum() / (g["volume"].astype(float).rolling(30).sum() + 1e-9))
        g_feat["vwap_30"] = vwap_30
        ...
        g_feat["bb_position"] = bb_position  # 0-1 position within bands
        g_feat["bb_width"] = (bb_upper - bb_lower) / (close + 1e-9)  # Normalized band width
        ...
        # OBV (volume trend) - используем diff вместо pct_change для стабильности
        volume = g["volume"].astype(float)
        obv = compute_obv(close, volume)
        obv_mean = obv.rolling(100, min_periods=10).mean().abs() + 1e-9
        g_feat["obv_change"] = obv.diff(10) / obv_mean  # Нормализованный OBV momentum
